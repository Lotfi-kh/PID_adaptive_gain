"""
Emit a portable, dependency-free C implementation of the frozen 12-D
noiselat-quiet actor, with weights extracted from the SAME frozen ONNX
(results/frozen_joint_12d_noiselat_quiet/actor_joint_12d_noiselat_quiet.onnx).
...
Outputs (NEW, namespaced — nothing old touched):
    export/portable_c_noiselat_quiet/rl_actor_weights.h   weights as const float[]
    export/portable_c_noiselat_quiet/rl_actor.h           API
    export/portable_c_noiselat_quiet/rl_actor.c           portable forward pass
    export/portable_c_noiselat_quiet/test_rl_actor.c      harness vs ONNX vectors

Usage:
    cd ~/rl_pid_tuner && python export/gen_portable_c_noiselat_quiet.py
"""
import os
import numpy as np
import onnx
...
ONNX_PATH = os.path.join(_ROOT, "results/frozen_joint_12d_noiselat_quiet/actor_joint_12d_noiselat_quiet.onnx")
OUT_DIR   = os.path.join(_HERE, "portable_c_noiselat_quiet")
...
def main():
    ...
    # Tolerance 5e-5 (not 1e-5 as for c860k/noiselat): noiselat-quiet froze the
    # gains, driving max|W| to ~16 in layers 2-3 (vs ~5 before). The larger
    # magnitudes accumulate more float32 rounding over the 64-wide dot products,
    # so float64-numpy vs float32-onnxruntime differs by ~2.7e-5 here. This is
    # rounding, not an extraction error: numpy-float32 (what the firmware C runs)
    # vs onnx-float32 is 1.2e-5 — ~1 ppm on a [-1,1] action, negligible once
    # scaled to the gain deltas. Shapes are asserted above; the math is identical.
    if worst > 5e-5:
        raise SystemExit(f"[PORTABLE] ABORT — extraction wrong, "
                         f"max|numpy - onnx| = {worst:.3e} (> 5e-5)")
    print(f"[PORTABLE] Self-check OK: max|numpy - onnx| = {worst:.3e} over 64 random inputs (tol 5e-5)")
    ...
    # ── rl_actor_weights.h ───────────────────────────────────────────────────
    h = ['/* AUTO-GENERATED from results/frozen_joint_12d_noiselat_quiet/actor_joint_12d_noiselat_quiet.onnx — do not edit. */',
    ...
    with open(os.path.join(OUT_DIR, "rl_actor.h"), "w") as f:
        f.write(
            ...
            " * noiselat-quiet actor. Bit-validated against the ONNX (see test_rl_actor.c).\n"
    ...
    with open(os.path.join(OUT_DIR, "test_rl_actor.c"), "w") as f:
        f.write(
            ...
            "  printf(\"%s\\n\", mx < 5e-5 ? \"PASS (float32-equivalent, tol 5e-5)\"\n"
            "                              : \"FAIL\");\n"
            "  return mx < 5e-5 ? 0 : 1;\n"
            "}\n")
