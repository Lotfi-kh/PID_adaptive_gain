...
=========================================================================
Tests the noiselat model across 48 conditions:
...
MODEL = os.environ.get("EVAL_MODEL", "/home/lotfikh/rl_pid_tuner/results/frozen_joint_12d_noiselat/td3_pid_noiselat_best.zip")
OUT_ROOT = os.environ.get("EVAL_OUT_ROOT", "/home/lotfikh/rl_pid_tuner/test_results/offnominal_eval_noiselat")
SCRIPT = "/home/lotfikh/rl_pid_tuner/eval_disturbance.py"
PYTHON = sys.executable

# Off-nominal + noise regime — match noiselat training (FROZEN_noiselat_12d.md):
# randomized mass/inertia + gyro/att noise 0.005 + 1-step control latency. This is
# noiselat's OWN distribution (the fair test), unlike the nominal aggressive eval.
SENSOR_NOISE_ATT = 0.005
SENSOR_NOISE_RATE = 0.005
CONTROL_LATENCY = 1
...
def run_condition(noise, mag_label, mag_val, dist_axis):
    ...
    cmd = [
        ...
        "--no-plots",
        "--randomize-dynamics",
        "--sensor-noise-att",      str(SENSOR_NOISE_ATT),
        "--sensor-noise-rate",     str(SENSOR_NOISE_RATE),
        "--control-latency-steps", str(CONTROL_LATENCY),
    ]
    t0 = time.time()
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"AGGRESSIVE EVALUATION — noiselat model   [{total} conditions × {EPISODES} episodes]")
    print(f"Model: {MODEL}")
    print(f"{'='*80}\n")
    ...
    report_lines = []
    report_lines.append(f"AGGRESSIVE EVAL SUMMARY — noiselat  [{total} conditions × {EPISODES} episodes]")
